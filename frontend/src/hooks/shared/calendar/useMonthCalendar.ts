import { getLastDateOfMonth } from '@/utils/shared';
  ...
  const handleSelectMonth = (currentDate: Date) => {
    ...
    if (selectedStartDate && !selectedEndDate) {
      if (newDate.getTime() > selectedStartDate.getTime()) {
        setSelectedEndDate(getLastDateOfMonth(newDate));
        return;
      }

      // 이전이면 기존 시작 월을 종료 날짜로 설정하고 새로운 시작 월을 설정
      setSelectedStartDate(newDate);
      setSelectedEndDate(getLastDateOfMonth(new Date(selectedStartDate)));
      return;
    }
